...
try:
    _STDIN_FILENO = sys.__stdin__.fileno() if sys.__stdin__ is not None else 0
except Exception:
    _STDIN_FILENO = 0
try:
    _STDOUT_FILENO = sys.__stdout__.fileno() if sys.__stdout__ is not None else 1
except Exception:
    _STDOUT_FILENO = 1
try:
    _STDERR_FILENO = sys.__stderr__.fileno() if sys.__stderr__ is not None else 2
except Exception:
    _STDERR_FILENO = 2
    ...
    def update(
        ...
        if renderables:
            if len(renderables) > 1:
                self.screen.renderable = Group(*renderables)
            else:
                # Safe access: we know renderables is truthy and has exactly 1 element
                self.screen.renderable = next(iter(renderables))
    ...
    def log(
        ...
        *objects: Any,
        ...
        with self:
            ...
            for hook in render_hooks:
                renderables = hook.process_renderables(cast(List[ConsoleRenderable], renderables))
            new_segments: List[Segment] = []
            extend = new_segments.extend
            render = self.render
